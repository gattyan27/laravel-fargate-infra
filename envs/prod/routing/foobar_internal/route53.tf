...
resource "aws_route53_record" "db_cname" {
  ...
  type    = "CNAME"
  ttl     = 300
  records = [data.terraform_remote_state.db_foobar.outputs.db_instance_this_address
  ]
}
...
resource "aws_route53_record" "cache_cname" {
  ...
  type    = "CNAME"
  ttl     = 300
  records = [data.terraform_remote_state.cache_foobar.outputs.elasticache_replication_group_this_primary_endpoint_address]
}